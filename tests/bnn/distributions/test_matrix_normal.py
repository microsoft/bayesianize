...
import models.bnn.distributions.matrix_normal as mn
...
@pytest.mark.parametrize("a_rows,a_cols,b_rows,b_cols", [
    (2, 3, 4, 5),
    (10, 12, 3, 6),
    (11, 3, 8, 19)
])
...
@pytest.mark.parametrize("x_batch,u_batch,v_batch", [
    (None, None, None),
    ((2,), 2, 2),
    ((2,), None, None),
    ((3, 2), 2, 2)
...
def test_sample_cov():
    ...
    assert torch.isclose(matnorm_cov.sub(true_cov).abs().mean(), multinorm_cov.sub(true_cov).abs().mean(), atol=1e-2)
