...
from models.bnn.bayesianize import bayesianize_, bayesian_from_template
...
def assert_conv_equal(l1, l2):
    assert all(getattr(l1, a) == getattr(l2, a) for a in
...
@pytest.mark.parametrize("torch_layer,inference,target_class", [
...
@pytest.mark.parametrize("inference,target_class", [
    ("ffg", bnn.nn.FFGLinear),
    ("fcg", bnn.nn.FCGLinear)
])
