...
def bayesianize_(network: nn.Module,
    ...
    for i, (name, module) in enumerate(network.named_modules()):
        ...
        if cls in _BAYESIAN_MODULES and module_inference["inference"] in _BAYESIAN_MODULES[cls]:
            ...
            if _module_in_sd(name, reference_state_dict):
                param_dict = {param_name: reference_state_dict[name + "." + param_name]
                              for param_name, _ in module.named_parameters()}
                bayesian_module.init_from_deterministic_params(param_dict)
        else:
            if _module_in_sd(name, reference_state_dict):
                module_sd = {
                    attr_name: reference_state_dict[name + "." + attr_name] for attr_name, _ in
                    itertools.chain(module.named_parameters(recurse=False), module.named_buffers(recurse=False))}
