...
class MatrixNormal(dist.Distribution):
    ...
    def __init__(self, loc, row_scale_tril, col_scale_tril, validate_args=None):
        ...
        if row_scale_tril.dim() < 2 or row_scale_tril.size(-1) != rows or row_scale_tril.size(-2) != rows:
            raise ValueError("row_scale_tril must be at-least two dimensional with the final two dimensions matching "
                             "the penultimate dimension of loc")
        if col_scale_tril.dim() < 2 or col_scale_tril.size(-1) != cols or col_scale_tril.size(-2) != cols:
            raise ValueError("col_scale_tril must be at-least two dimensional with the final two dimensions matching "
    ...
    @property
    def variance(self):
        return self.row_scale_tril.pow(2).sum(-1).unsqueeze(-1).mul(
            self.col_scale_tril.pow(2).sum(-1)).expand(self._batch_shape + self._event_shape)
